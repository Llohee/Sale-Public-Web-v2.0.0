    ...
    return (
      ...
      >
        <motion.div
          ...
          className={cn(
            "dock mx-auto flex justify-center h-16 items-end gap-2 rounded-t-2xl bg-oregon-800/85 px-3 pb-2 backdrop-blur-md",
            "",
            className,
          )}
        >
...
const DockItem = React.forwardRef<HTMLDivElement, DockItemProps>(
  ({ className, children, onClick, active = false }, _ref) => {
    ...
    return (
      <motion.div
        ref={itemRef}
        style={{
          width: active ? iconSize * maxScale : width,
          height: active ? iconSize * maxScale : width,
        }}
        onClick={onClick}
        data-active={active}
        className={cn(
          "dock-item group relative flex aspect-square cursor-pointer items-center justify-center rounded-xl bg-muted transition-colors hover:bg-muted/80 data-[active=true]:bg-white",
          className,
        )}
      >
        <div className={`rounded-xl transition-all p-4 ${active ? "bg-orange-900 rounded-full" : ""}`}>
          {children}
        </div>
      </motion.div>
    );
